...
class AdsBox extends React.Component<Props, State> {
  ...
  onAdsSettingChange = (key: string, value: string) => {
    ...
    const { adsEnabled, shouldOptOutOfAdConversions } = this.props.rewardsData.adsData

    if (key === 'adsEnabled') {
      newValue = !adsEnabled
    } else if (key === 'shouldOptOutOfAdConversions') {
      newValue = !shouldOptOutOfAdConversions
    }
  ...
  adsSettings = (enabled?: boolean) => {
    ...
    const { adsPerHour, shouldOptOutOfAdConversions } = this.props.rewardsData.adsData
    ...
    return (
      <Grid columns={1} customStyle={{ margin: '0 auto' }}>
        <Column size={1} customStyle={{ justifyContent: 'center', flexWrap: 'wrap' }}>
          ...
          <ControlWrapper text={getLocale('adsOptOutOfAdConversions')}>
            <Toggle
              onToggle={this.onAdsSettingChange.bind(this, 'shouldOptOutOfAdConversions', '')}
              checked={shouldOptOutOfAdConversions}
            />
          </ControlWrapper>
        </Column>
      </Grid>
