...
void RewardsDOMHandler::OnGetMonthlyReport(
  ...
  for (const auto& item : report.transactions) {
    ...
    publisher.SetStringKey("id", item.publisher.id);
    publisher.SetStringKey("publisherKey", item.publisher.id);
    publisher.SetIntKey("status", item.publisher.status);
    publisher.SetStringKey("name", item.publisher.name);
    publisher.SetStringKey("provider", item.publisher.provider);
    publisher.SetStringKey("url", item.publisher.url);
    publisher.SetStringKey("favIcon", item.publisher.favicon_url);
    ...
    transaction_report.SetKey("publisher", std::move(publisher));
